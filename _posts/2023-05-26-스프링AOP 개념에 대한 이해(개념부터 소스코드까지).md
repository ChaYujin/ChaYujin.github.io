...
AOP의 개념부터 스프링 코드까지 훑어보는 시간을 가졌다. 다시 한번 간단하게 정리해보자면, AOP는 객체지향 언어의 한계를 보완하기위해 고안된 프로그래밍 기법이며, 스프링 AOP는 프록시를 바탕으로 만들어졌다. 정말 많은 설명을 했지만, 이 내용이 핵심 내용이다. 프록시 설명을 위해 긴 설명이 필요했던 것이다.
